...
use crate::reachability_constraints::is_range_reachable;
...
use ty_semantic_index::{attribute_scopes, global_scope, semantic_index, use_def_map};
...
pub fn definitions_for_name<'db>(
    ...
    for (scope_id, _scope) in index.visible_ancestor_scopes(file_scope) {
        ...
        if is_global {
            ...
            let global_place_table = ty_semantic_index::place_table(db, global_scope_id);

            if let Some(global_symbol_id) = global_place_table.symbol_id(name_str) {
                let global_use_def_map = ty_semantic_index::use_def_map(db, global_scope_id);
                let global_bindings =
...
fn definitions_for_attribute_in_class_hierarchy<'db>(
        ...
        .filter_map(|cls: ClassType<'db>| cls.static_class_literal(db).map(|(lit, _)| lit))
    {
        ...
        let class_place_table = ty_semantic_index::place_table(db, class_scope);
...
mod resolve_definition {
    ...
    use crate::Db;
    use ty_semantic_index::definition::{Definition, DefinitionKind, module_docstring};
    use ty_semantic_index::scope::{NodeWithScopeKind, ScopeId};
    use ty_semantic_index::{global_scope, place_table, semantic_index, use_def_map};
